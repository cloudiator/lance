...
public class RewiringTestImpl extends TestImpl implements RewiringTestAgent {
  ...
  private static void assertRightState(
      ContainerStatus stat, ErrorAwareContainer<PlainContainerLogic> container) {
    for (ContainerStatus status : ContainerStatus.values()) {
      if (status == stat) assert status == container.getState();
      else assert status != container.getState();
    }
  }

  @Override
  public ApplicationInstanceId testNewTopology(
  ...
  protected void init(AppArchitecture arch) throws ContainerException {
    ...
    for (ComponentInfo cInfo : arch.getComponents()) {
      // DeployableComponent.Builder builder = new
      // Builder();.createBuilder(arch.getApplicationName(), cInfo.getComponentId());
      DeployableComponent.Builder builder =
          DeployableComponent.Builder.createBuilder(
              arch.getApplicationName(), cInfo.getComponentId());
      boolean stopIt;
      ...
      for (InportInfo inInf : cInfo.getInportInfos())
        builder.addInport(
            inInf.getInportName(), inInf.getPortType(), inInf.getCardinality(), inInf.getInPort());

      for (OutportInfo outInf : cInfo.getOutportInfos())
        builder.addOutport(
            outInf.getOutportName(),
            outInf.getPuHandler(),
            outInf.getCardinality(),
            outInf.getMin());

      builder.deploySequentially(true);
      DeployableComponent comp = builder.build();
      GlobalRegistryAccessor accessor =
          new GlobalRegistryAccessor(core.ctx, comp, cInfo.getComponentInstanceId());
      final LcaRegistry reg = core.ctx.getRegistry();
      NetworkHandler networkHandler =
          new NetworkHandler(accessor, comp, CoreElementsRemote.context);
  ...
  private ErrorAwareContainer<PlainContainerLogic> createNewContainer(FullComponent fullComp)
    ...
    builder =
        builder
            .cInstId(fullComp.cInstId)
            .deplComp(fullComp.comp)
            .deplContext(core.ctx)
            .operatingSys(fullComp.exCtx.getOperatingSystem());
    PlainContainerLogic containerLogic =
        builder
            .nwHandler(fullComp.networkHandler)
            .plShellFac(plainShellFactory)
            .hostContext(core.context)
            .build();

    ExecutionContext executionContext =
        new ExecutionContext(fullComp.exCtx.getOperatingSystem(), plainShellFactory);
    LifecycleController lifecycleController =
        new LifecycleController(
            fullComp.comp.getLifecycleStore(),
            containerLogic,
            fullComp.accessor,
            executionContext,
            core.context);
  ...
  private LifecycleStore createKafkaLifecycleStore() {
    ...
    store.setHandler(
        builder_pre_inst.build(LifecycleHandlerType.PRE_INSTALL), LifecycleHandlerType.PRE_INSTALL);
    ...
    store.setHandler(
        builder_inst.build(LifecycleHandlerType.INSTALL), LifecycleHandlerType.INSTALL);
    ...
    store.setHandler(
        builder_post_inst.build(LifecycleHandlerType.POST_INSTALL),
  ...
  private LifecycleStore createCassLifecycleStore() {
    ...
    store.setHandler(
        builder_pre_stop.build(LifecycleHandlerType.PRE_STOP), LifecycleHandlerType.PRE_STOP);
