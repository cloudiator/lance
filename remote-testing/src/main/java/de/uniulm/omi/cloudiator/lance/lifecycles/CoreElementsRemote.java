...
public class CoreElementsRemote {
  ...
  private static final String cloudIp = "127.0.0.1";

  public static volatile AppArchitecture arch;
  ...
  private void setPortInfoContext() {
    // todo: extend, so that multiple outports can be required by this inport ->
    // Map<Integer,List<InportInfo> requiredPortsMap & adjust @2nd inner loop below
  ...
  private void setInportInfoContext(
      ComponentInfo cInf,
      Map<Integer, ComponentId> requiredComponentsMap,
      Map<Integer, InportInfo> requiredPortsMap) {
  ...
  private void setOutportInfoContext(
      ComponentInfo cInf,
      Map<Integer, ComponentId> requiredComponentsMap,
      Map<Integer, InportInfo> requiredPortsMap) {
    for (OutportInfo outInf : cInf.getOutportInfos()) {
      if (outInf.getMin() != OutPort.NO_SINKS
          && requiredPortsMap.get(outInf.getProvidedPortNumber()) != null) {
        ...
        ctx.setProperty(
            ...
            (Object)
                new PortReference(
                    compId, portRefInfo.getInportName(), PortProperties.PortLinkage.ALL),
            OutPort.class);
      }
    }
  }
  ...
  public void setUpRegistry() throws RegistrationException {
    reg.addApplicationInstance(
        arch.getAppInstanceId(), arch.getApplicationId(), arch.getApplicationName());
  }
  ...
  public Map<ComponentInstanceId, Map<String, String>> checkBasicRegistryValues(ComponentId cId)
