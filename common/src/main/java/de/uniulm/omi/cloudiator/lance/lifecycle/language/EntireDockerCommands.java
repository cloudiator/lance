...
public class EntireDockerCommands implements Serializable {
  ...
  public static void copyCmdOptions(DockerCommand from, DockerCommand to)
  ...
  public static void copyCmdOsCommand(DockerCommand from, DockerCommand to)
      throws DockerCommandException {
    if (!(from.cmdType == to.cmdType)
        && !((from.cmdType == Type.CREATE) && (to.cmdType == Type.RUN)))
      throw new DockerCommandException("Cannot convert from " + from.cmdType + " to " + to.cmdType);
  ...
  public static void copyCmdArgs(DockerCommand from, DockerCommand to)
      throws DockerCommandException {
    if (!(from.cmdType == to.cmdType)
        && !((from.cmdType == Type.CREATE) && (to.cmdType == Type.RUN)))
      throw new DockerCommandException("Cannot convert from " + from.cmdType + " to " + to.cmdType);
  ...
  public void setOption(DockerCommand.Type cType, Option opt, String arg)
      throws DockerCommandException {
    if (cType == DockerCommand.Type.CREATE) create.setOption(opt, arg);
    else if (cType == DockerCommand.Type.START) start.setOption(opt, arg);
    else if (cType == DockerCommand.Type.STOP) stop.setOption(opt, arg);
    else if (cType == DockerCommand.Type.RUN) run.setOption(opt, arg);
    else if (cType == DockerCommand.Type.REMOVE) remove.setOption(opt, arg);
    else throw new DockerCommandException(wrongCmdMessage);
  }

  public void setOsCommand(DockerCommand.Type cType, OsCommand osCmd)
      throws DockerCommandException {
    if (cType == DockerCommand.Type.CREATE) create.setOsCommand(osCmd);
    else if (cType == DockerCommand.Type.START) start.setOsCommand(osCmd);
    else if (cType == DockerCommand.Type.STOP) stop.setOsCommand(osCmd);
    else if (cType == DockerCommand.Type.RUN) run.setOsCommand(osCmd);
    else if (cType == DockerCommand.Type.REMOVE) remove.setOsCommand(osCmd);
    else throw new DockerCommandException(wrongCmdMessage);
  }

  public void setArg(DockerCommand.Type cType, String arg) throws DockerCommandException {
    if (cType == DockerCommand.Type.CREATE) create.setArg(arg);
    else if (cType == DockerCommand.Type.START) start.setArg(arg);
    else if (cType == DockerCommand.Type.STOP) stop.setArg(arg);
    if (cType == DockerCommand.Type.RUN) run.setArg(arg);
    if (cType == DockerCommand.Type.REMOVE) remove.setArg(arg);
    else throw new DockerCommandException(wrongCmdMessage);
  }

  public String getSetOptionsString(DockerCommand.Type cType) throws DockerCommandException {
    if (cType == DockerCommand.Type.CREATE) return create.getSetOptionsString();
    else if (cType == DockerCommand.Type.START) return start.getSetOptionsString();
    else if (cType == DockerCommand.Type.STOP) return stop.getSetOptionsString();
    else if (cType == DockerCommand.Type.RUN) return run.getSetOptionsString();
    else if (cType == DockerCommand.Type.REMOVE) return remove.getSetOptionsString();
    else throw new DockerCommandException(wrongCmdMessage);
  }

  public String getSetOsCommandString(DockerCommand.Type cType) throws DockerCommandException {
    if (cType == DockerCommand.Type.CREATE) return create.getSetOsCommandString();
    if (cType == DockerCommand.Type.START) return start.getSetOsCommandString();
    if (cType == DockerCommand.Type.STOP) return stop.getSetOsCommandString();
    if (cType == DockerCommand.Type.RUN) return run.getSetOsCommandString();
    if (cType == DockerCommand.Type.REMOVE) return remove.getSetOsCommandString();
    else throw new DockerCommandException(wrongCmdMessage);
  }

  public String getSetArgsString(DockerCommand.Type cType) throws DockerCommandException {
    if (cType == DockerCommand.Type.CREATE) return create.getSetArgsString();
    if (cType == DockerCommand.Type.START) return start.getSetArgsString();
    if (cType == DockerCommand.Type.STOP) return stop.getSetArgsString();
    if (cType == DockerCommand.Type.RUN) return run.getSetArgsString();
    if (cType == DockerCommand.Type.REMOVE) return remove.getSetArgsString();
    else throw new DockerCommandException(wrongCmdMessage);
  }
  ...
  public String getContainerName(DockerCommand.Type cType) throws DockerCommandException {
    if (cType == DockerCommand.Type.CREATE) return create.getContainerName();
    if (cType == DockerCommand.Type.START) return start.getContainerName();
    if (cType == DockerCommand.Type.STOP) return stop.getContainerName();
    if (cType == DockerCommand.Type.RUN) return run.getContainerName();
    if (cType == DockerCommand.Type.REMOVE) return remove.getContainerName();
    else throw new DockerCommandException(wrongCmdMessage);
  }
  ...
  public static class Builder {
    ...
    public Builder setOptions(DockerCommand.Type type, Map<Option, List<String>> opts)
        throws DockerCommandException {
      if (type == Type.CREATE) {
    ...
    public Builder setCommand(DockerCommand.Type type, List<OsCommand> cmd)
        throws DockerCommandException {
      if (type == Type.CREATE) {
    ...
    public Builder setArgs(DockerCommand.Type type, List<String> args)
        throws DockerCommandException {
      if (type == Type.CREATE) {
