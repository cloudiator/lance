...
public class DockerCommand implements Serializable {
  ...
  private static void checkMapKeysInSet(Map<Option,List<String>> map, Set<Option> set, DockerCommand.Type cType) throws DockerCommandException {
    for (Map.Entry<Option, List<String>> kv : map.entrySet()) {
      if(!set.contains(kv.getKey()))
        throw new DockerCommandException("Option " + kv.getKey().name() + " does not exist for '" + DockerCommand.Type.mapCommandToString(cType) + "' command");
    }
  }
