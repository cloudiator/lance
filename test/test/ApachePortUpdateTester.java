...
import de.uniulm.omi.cloudiator.lance.lca.container.docker.DockerShell;
import de.uniulm.omi.cloudiator.lance.lca.container.docker.connector.ConnectorFactory;
import de.uniulm.omi.cloudiator.lance.lca.container.docker.connector.DockerConnector;
import de.uniulm.omi.cloudiator.lance.lca.container.docker.connector.DockerException;
...
public final class ApachePortUpdateTester {
	...
	private static final String MAIN_COMMAND = "for el in $(echo \"$" + APACHELB_OUT_PORT_NAME + "\" | sed -e \"s/,/\\n/g\") ; do " +
	...
	private static final String TEST_COMMAND_0 = "$(echo \"$" + APACHELB_OUT_PORT_NAME + "\" | sed -e \"s/,/\\n/g\")"; 
	private static final String TEST_COMMAND_1 = "for el in $(echo \"$" + APACHELB_OUT_PORT_NAME + "\" | sed -e \"s/,/\\n/g\") ; do " +
			"touch /tmp/$el ; done";
		
	public static void main(String[] main) throws DockerException {
		
		ComponentInstanceId myId = ComponentInstanceId.fromString("db5f920e-1404-44eb-9a9d-68d9f375cbdd");
		DockerConnector client = ConnectorFactory.INSTANCE.createConnector("local");
		DockerShell dshell = client.getSideShell(myId);
		
		ExecutionResult result = dshell.executeCommand("export " +APACHELB_OUT_PORT_NAME + "=192.168.1.4:8000,192.168.1.5:8900" );
		result = dshell.executeCommand("env");
		result = dshell.executeCommand("sed -i -e \"/.*# Generated line/d\" " + UBUNTU_CONFIG_LOCATION + "/mods-available/" + UBUNTU_PROXY_CONFIG_FILE);
