...
public final class ErrorAwareStateMachine<T extends Enum<?> & State> {
  ...
  static final Logger LOGGER = LoggerFactory.getLogger(ErrorAwareStateMachine.class);
  ...
  ErrorAwareStateMachine(
      T init, T genericErrorState, List<T> states, List<ErrorAwareStateTransition<T>> st) {
   ...
   * @param params set of application specific parameters to be passed through to the transition
   */
