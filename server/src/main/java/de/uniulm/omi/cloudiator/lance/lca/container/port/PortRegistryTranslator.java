...
public final class PortRegistryTranslator {
  ...
  public static final PortHierarchyLevel PORT_HIERARCHY_0 =
      PortHierarchyLevel.create(PORT_HIERARCHY_0_NAME);
  private static final String PORT_HIERARCHY_1_NAME = "CLOUD";
  public static final PortHierarchyLevel PORT_HIERARCHY_1 =
      PortHierarchyLevel.create(PORT_HIERARCHY_1_NAME);
  ...
  private static Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>>
      getHierarchicalPorts(
          PortReference sinkReference,
          Map<ComponentInstanceId, Map<String, String>> dump,
          PortHierarchy portHierarchy)
    ...
    for (Entry<ComponentInstanceId, Map<String, String>> entry : dump.entrySet()) {
      ...
      boolean isReady =
          GlobalRegistryAccessor.dumpMapHasContainerStatus(map, ContainerStatus.READY);
      if (!isReady) {
        LOGGER.info(
            "dropping data (ports and ips of component instance "
                + id
                + " as it is not in ready state");
        continue;
      }
      HierarchyLevelState<DownstreamAddress> state =
          new HierarchyLevelState<>(id.toString(), portHierarchy);
      boolean forAll = true;
  ...
  private static Integer getHierarchicalPort(
      PortReference sinkReference, Map<String, String> dump, PortHierarchyLevel level)
      throws RegistrationException {
  ...
  private static String getHierarchicalHostname(PortHierarchyLevel level, Map<String, String> dump)
      throws RegistrationException {
  ...
  public Integer findPortAtLevel(
      ComponentInstanceId myId, String portName, PortHierarchyLevel level)
      throws RegistrationException {
    String key = buildFullPortName(portName, level);
    try {
  ...
  public Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> findDownstreamInstances(
    ...
    Map<ComponentInstanceId, Map<String, String>> dump =
        accessor.retrieveComponentDump(sinkReference);
