...
public final class PortRegistryTranslator {
  ...
  public static final PortHierarchyLevel PORT_HIERARCHY_0 = PortHierarchyLevel
      .create(PORT_HIERARCHY_0_NAME);
  public static final PortHierarchyLevel PORT_HIERARCHY_1 = PortHierarchyLevel
      .create(PORT_HIERARCHY_1_NAME);
  public static final PortHierarchyLevel PORT_HIERARCHY_2 = PortHierarchyLevel
      .create(PORT_HIERARCHY_2_NAME);

  public static final PortHierarchy PORT_HIERARCHY = new PortHierarchyBuilder()
  ...
  public void registerLocalPortAtLevel(String portName, PortHierarchyLevel level, Integer value)
      throws RegistrationException {
    String key = buildFullPortName(portName, level);
    accessor.addLocalProperty(key, value.toString());
  }

  public Integer findPortAtLevel(ComponentInstanceId myId, String portName,
      PortHierarchyLevel level) throws RegistrationException {
    String key = buildFullPortName(portName, level);
    try {
  ...
  public void registerLocalAddressAtLevel(PortHierarchyLevel level, String value)
      throws RegistrationException {
  ...
  public Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> findDownstreamInstances(
      OutPort out, PortHierarchy portHierarchy) throws RegistrationException {
    ...
    try {
      ...
      throw new IllegalStateException(
          "sink unknown: port '" + out.getName() + "' not correctly wired.", cce);
    }

    Map<ComponentInstanceId, Map<String, String>> dump = accessor
        .retrieveComponentDump(sinkReference);
  ...
  private static Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> getHierarchicalPorts(
      PortReference sinkReference, Map<ComponentInstanceId, Map<String, String>> dump,
      PortHierarchy portHierarchy) throws RegistrationException {
    ...
    for (Entry<ComponentInstanceId, Map<String, String>> entry : dump.entrySet()) {
      ...
      boolean isReady = GlobalRegistryAccessor
          .dumpMapHasContainerStatus(map, ContainerStatus.READY);
      if (!isReady) {
        LOGGER.info("dropping data (ports and ips of component instance " + id
            + " as it is not in ready state");
        continue;
      }
      HierarchyLevelState<DownstreamAddress> state = new HierarchyLevelState<>(id.toString(),
          portHierarchy);
      boolean forAll = true;
  ...
  private static Integer getHierarchicalPort(PortReference sinkReference, Map<String, String> dump,
      PortHierarchyLevel level) throws RegistrationException {
    ...
    try {
      if (value == null) {
        ...
        throw new RegistrationException(
            "port with '" + key + "' has not been found. Value was null.");
      }
      Integer i = Integer.valueOf(value);
      if (isValidPortOrUnset(i)) {
        return i;
      ...
      throw new RegistrationException(
          "(" + key + ") " + " value was not an expected number: " + value, nfe);
    }
  }

  public static String getHierarchicalHostname(PortHierarchyLevel level, Map<String, String> dump)
      throws RegistrationException {
    String key = buildFullHostName(level);
    String value = dump.get(key);
    if (value == null) {
      throw new RegistrationException(
          "ipaddress for '" + key + "' has not been found. Value was null.");
    }
    ...
    try {
      ...
      try {
        ...
        throw new RegistrationException("neither valid URL nor IP address: " + value, uhe);
      }
    }
