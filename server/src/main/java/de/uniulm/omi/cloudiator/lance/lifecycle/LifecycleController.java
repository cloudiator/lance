...
public final class LifecycleController {
  ...
  public LifecycleController(
      ...
      LifecycleActionInterceptor interceptorParam,
    ...
    hostContext = hostContextParam;
    stopDetector =
        StopDetectorHandler.create(interceptorParam, store.getStopDetector(), ecParam, callback);
  }
  ...
  public synchronized void blockingInstall() {
    run(
        ...
        LifecycleHandlerType.PRE_INSTALL); // executes pre_install handler
  ...
  public synchronized void blockingConfigure() {
    run(
        ...
        LifecycleHandlerType.POST_INSTALL); // executes POST_INSTALL handler
    run(
        LifecycleHandlerType.POST_INSTALL,
        LifecycleHandlerType.PRE_START); // executes PRE_START handler
  }
  ...
  public synchronized void blockingUpdatePorts(
      OutPort port, PortUpdateHandler handler, PortDiff<DownstreamAddress> diff)
    ...
    try {
      ...
      getLogger()
          .warn("Exception when executing state transition. this is not thoroughly handled.", ce);
      throw new IllegalStateException("wrong state: should be in error state?", ce);
      // set error state
