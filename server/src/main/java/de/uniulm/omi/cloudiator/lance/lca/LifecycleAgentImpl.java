...
public class LifecycleAgentImpl implements LifecycleAgent {
  ...
  LifecycleAgentImpl(HostContext contex) {
    ...
    // manager = ContainerManagerFactory.createContainerManager(contex,
    // ContainerType.fromString(contex.getContainerType()));
    this.hostContext = contex;
    status = AgentStatus.CREATED;
  }

  private static void componentConsistentlyRegistered(
      DeploymentContext ctx, AbstractComponent component)
  ...
  private static void doRunContainer(
    ...
    hostContext_.run(
        () -> {
          try {
            runPreInit(cc);
            LOGGER.info(String.format("Initiating lifecycle store of component %s.", component_));
            cc.init(store);
            LOGGER.info(String.format("Awaiting Initialisation of %s.", cc));
            cc.awaitInitialisation();
          } catch (ContainerException e) {
            LOGGER.error(
                "Error occurred in container controller " + cc + " for Lifecycle Component", e);
          } catch (Exception e) {
            LOGGER.error(
                "Unexpected exception occured in container controller "
                    + cc
                    + " for Lifecycle Component",
                e);
          }
        });
  }
  ...
  private static void doRunContainer(
      ContainerController cc, HostContext hostContext_, AbstractComponent component_) {
    hostContext_.run(
        () -> {
          try {
            runPreInit(cc);
            LOGGER.info(
                String.format("Initiating component %s without lifecylce store.", component_));
            cc.init();
            LOGGER.info(String.format("Awaiting Initialisation of %s.", cc));
            cc.awaitInitialisation();
          } catch (ContainerException e) {
  ...
  private static void componentPartOfApplication(DeploymentContext ctx, AbstractComponent component)
      throws RegistrationException, LcaException {
    LcaRegistry reg = ctx.getRegistry();
    ApplicationInstanceId appInstId = ctx.getApplicationInstanceId();
    ...
    throw new LcaException(
        String.format(
            "cannot proceed: component %s is not known within application instance %s.",
            component, appInstId));
  }
  ...
  @Override
  public ComponentInstanceId deployDeployableComponent(
    ...
    ContainerManager manager =
        containers.getLifecycleContainerManager(hostContext, os, containerType);

    LOGGER.info(
        String.format(
            "Creating new container using context %s, os %s and containerType %s.",
            hostContext, os, containerType));
    ContainerController cc = manager.createNewLifecycleContainer(ctx, component, os, false);

    LOGGER.info(
        String.format("Dispatching handling of container controller %s to execution handler.", cc));
  ...
  @Override
  public ComponentInstanceId deployDockerComponent(DeploymentContext ctx, DockerComponent component)
      throws RemoteException, LcaException, RegistrationException, ContainerException {
    ...
    LOGGER.info(
        String.format(
            "Creating new container using context %s, and containerType %s.",
  ...
  @Override
  public ComponentInstanceId deployRemoteDockerComponent(
  ...
  @Override
  public boolean stopComponentInstance(ComponentInstanceId instanceId, boolean forceRegDel)
  ...
  @Override
  public boolean componentInstanceIsReady(ComponentInstanceId instanceId)
      throws RemoteException, LcaException, ContainerException {
    ContainerController cid = getController(instanceId);
    ContainerStatus state = cid.getState();
