...
public final class OutPortState {
   ...
   * @return true if the port of the OutPort of the diff set matches the OutPort maintained by this
   *     OutPortState.
   */
  boolean enactDiffSet(PortDiff<DownstreamAddress> diff) {
    ...
    if (!oldSinks.equals(compOldSinks)) {
      LOGGER.warn(
          "old sinks do not match; do we have concurrency pvroblems?"
              + oldSinks
              + " vs "
              + compOldSinks);
    }
  ...
  @Deprecated
  PortDiff<DownstreamAddress> updateWithDiff(
      Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> newSinks) {
    Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> oldSinks = getCurrentSinkSet();
    PortDiff<DownstreamAddress> diff = new PortDiff<>(newSinks, oldSinks, thePort);
  ...
  public boolean requiredAndSet() {
    ...
    LOGGER.debug(
        String.format(
            "Evaluating require and set - lower bound: %s; sinks: %s. Return %s.",
