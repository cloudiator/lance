...
final class DockerImageHandler {
  ...
  DockerImageHandler(
      ...
      DockerOperatingSystemTranslator translatorParam,
      DockerConnector clientParam,
      AbstractComponent componentParam,
  ...
  private String buildImageTagName(
      ImageCreationType type, String componentInstallId, String imageName) {
    final String key;
    switch (type) {
      case COMPONENT_INSTANCE:
  ...
  private String imageFromComponent(String componentInstallId) {
    ...
    if (!dockerConfig.registryCanBeUsed()) return tmp;
    return dockerConfig.prependRegistry(tmp);
  }

  // TODO: Maybe allow several writings for the image name (e.g. "cassandra", "cassandra 12.2.2",
  // "cassandra:12.2.2", "cassandra latest", "cassandra:latest") and resolve via regex check
  ...
  private String getImageFromPrivateRepository(String imageName) {
    ...
    try {
      if (!useCredentials) {
        ...
        RegistryAuth registryAuth =
            RegistryAuth.builder()
                ...
                .password(password)
                .serverAddress(
                    hostName + (!(port < 0) && port < 65555 ? ":" + Integer.toString(port) : ""))
                .build();

        DockerClient dClient =
            DefaultDockerClient.fromEnv().dockerAuth(false).registryAuth(registryAuth).build();
      ...
      LOGGER.error(
          "Problems with DockerClient. Cannot pull image "
              + regExpandedImageName
