 ...
 */
final class ErrorAwareStateMachineState<T extends Enum<?> & State > implements ErrorAwareTransitionState<T> {
    ...
    ErrorAwareStateMachineState(T initState, T genericErrorStateParam, Stack<Throwable> exceptionStore) {
    	status = initState;
	...
	TransitionHandle<T> getTransitionHandle() {
		if(!hasOngoingTransition(false)){
			throw new IllegalStateException("transition handle only available when transitions are ongoing.");
		}
