...
final class OutPortHandler implements DynamicEnvVars {
  ...
  private static Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>>
      filterInstancesWithUnsetPorts(
          Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> instances) {
    Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> retVal = new HashMap<>();
    for (Entry<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> entry :
        instances.entrySet()) {
      boolean doSet = true;
  ...
  private static Map<PortHierarchyLevel, List<DownstreamAddress>> doCollect(
      OutPortState out, Map<PortHierarchyLevel, List<DownstreamAddress>> elements) {
  ...
  void initPortStates(PortRegistryTranslator accessor, PortHierarchy portHierarchy)
    ...
    for (OutPort out : outPorts) {
      Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> instances =
          accessor.findDownstreamInstances(out, portHierarchy);
   ...
   * @throws RegistrationException
   */
  List<PortDiff<DownstreamAddress>> getUpdatedDownstreamPorts(
      PortRegistryTranslator accessor, PortHierarchy portHierarchy) throws RegistrationException {
    List<PortDiff<DownstreamAddress>> changedPorts = new LinkedList<>();
    for (OutPortState outPort : portStates) {
      Map<ComponentInstanceId, HierarchyLevelState<DownstreamAddress>> instances =
          accessor.findDownstreamInstances(outPort.getPort(), portHierarchy);
  ...
  void updateDownstreamPorts(PortRegistryTranslator accessor, PortHierarchy portHierarchy)
      throws RegistrationException {
  ...
  public void generateDynamicEnvVars(PortDiff<DownstreamAddress> diffSet) {
    for (OutPortState out : portStates) {
      ...
      Map<PortHierarchyLevel, List<DownstreamAddress>> toGenerate =
          elements.isEmpty() ? doCollect(out, EMPTY_VISIT_MAP) : doCollect(out, elements);
      doGenerateDynamicEnvVars(out, toGenerate);
    }
  }

  private void doGenerateDynamicEnvVars(
      OutPortState out, Map<PortHierarchyLevel, List<DownstreamAddress>> toGenerate) {
