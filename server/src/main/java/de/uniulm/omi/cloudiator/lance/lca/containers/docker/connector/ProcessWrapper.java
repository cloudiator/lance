...
public final class ProcessWrapper {
  ...
  private ProcessWrapper(
      Process procParam,
      OutputStream stdinParam,
      InputStream stdoutParam,
      InputStream stderrParam) {
    proc = procParam;
  ...
  private static ProcessWrapper createWrappper(ProcessBuilder pb) {
    try {
      ...
      return new ProcessWrapper(
          proc, proc.getOutputStream(), proc.getInputStream(), proc.getErrorStream());
  ...
  private static List<String> argsAsDockerList(String... args) {
    ...
    // If a quote appears -> treat all following Strings (incl. the quote) until next quote appears
    // as one arg
    // including the quote
