...
public class DockerContainerManager implements ContainerManager {
  ...
  public DockerContainerManager(HostContext vmId, RemoteDockerComponent.DockerRegistry dReg) {
    ...
    System.setProperty(DockerConfigurationFields.DOCKER_REGISTRY_PORT_KEY,
        Integer.toString(dReg.port));
  ...
  public ContainerController createNewLifecycleContainer(DeploymentContext ctx,
      DeployableComponent comp, OperatingSystem os, boolean shouldBeRemoved)
      throws ContainerException {

    //todo: implement this also for LifecycleContainers!?
    LifecycleContainerComponents cComponents = new LifecycleContainerComponents(comp, hostContext,
        client, ctx, dockerConfig, os);
    accessorInit(cComponents.accessor, cComponents.id);
    ContainerController dc = new ErrorAwareContainer<>(cComponents.id, cComponents.logic,
        cComponents.networkHandler, cComponents.controller, cComponents.accessor, shouldBeRemoved);
  ...
  public ContainerController createNewDockerContainer(DeploymentContext ctx,
    ...
    DockerContainerComponents cComponents = new DockerContainerComponents(comp, hostContext, client,
        ctx, dockerConfig);
    accessorInit(cComponents.accessor, cComponents.id);
    ContainerController dc = new ErrorAwareContainer<>(cComponents.id, cComponents.logic,
        cComponents.networkHandler, cComponents.controller, cComponents.accessor, shouldBeRemoved);
  ...
  public ContainerController createNewRemoteDockerContainer(DeploymentContext ctx,
    ...
    RemoteDockerContainerComponents cComponents = new RemoteDockerContainerComponents(comp,
        hostContext, client, ctx, dockerConfig);
    accessorInit(cComponents.accessor, cComponents.id);
    ContainerController dc = new ErrorAwareContainer<>(cComponents.id, cComponents.logic,
        cComponents.networkHandler, cComponents.controller, cComponents.accessor, shouldBeRemoved);
    registry.addContainer(dc);
  ...
  private static class LifecycleContainerComponents extends DefaultContainerComponents {
    ...
    LifecycleContainerComponents(DeployableComponent comp, HostContext hostContext,
      ...
      this.logic = builder.cInstId(id).dockerConnector(client).deplComp(comp).deplContext(ctx).
          ...
          .hostContext(hostContext).build();

      ExecutionContext ec = new ExecutionContext(os, shellFactory);
      this.controller = new LifecycleController(comp.getLifecycleStore(), logic, accessor, ec,
          hostContext);
    }
  }
  ...
  private static class DefaultDockerContainerComponents extends DefaultContainerComponents {
    ...
    DefaultDockerContainerComponents(AbstractComponent comp, HostContext hostContext,
        DockerConnector client,
  ...
  private static class RemoteDockerContainerComponents extends DefaultDockerContainerComponents {
    ...
    RemoteDockerContainerComponents(RemoteDockerComponent comp, HostContext hostContext,
        DockerConnector client,
        DeploymentContext ctx, DockerConfiguration dockerConfig) {
      super(comp, hostContext, client, ctx, dockerConfig);
