...
public class DockerContainerManager implements ContainerManager {
  ...
  public DockerContainerManager(HostContext vmId, RemoteDockerComponent.DockerRegistry dReg) {
    ...
    System.setProperty(
        DockerConfigurationFields.DOCKER_REGISTRY_PORT_KEY, Integer.toString(dReg.port));
  ...
  DockerContainerManager(
      HostContext vmId, String host, boolean remote, DockerConfiguration config) {
    hostContext = vmId;
    hostname = host;
    client = ConnectorFactory.INSTANCE.createConnector(hostname);
    // translator = createAndInitTranslator();
  ...
  private static void accessorInit(GlobalRegistryAccessor acc, ComponentInstanceId cId)
  ...
  @Override
  public ContainerController createNewLifecycleContainer(
      DeploymentContext ctx, DeployableComponent comp, OperatingSystem os, boolean shouldBeRemoved)
      throws ContainerException {

    // todo: implement this also for LifecycleContainers!?
    LifecycleContainerComponents cComponents =
        new LifecycleContainerComponents(comp, hostContext, client, ctx, dockerConfig, os);
    accessorInit(cComponents.accessor, cComponents.id);
    ContainerController dc =
        new ErrorAwareContainer<>(
            cComponents.id,
            cComponents.logic,
            cComponents.networkHandler,
            cComponents.controller,
            cComponents.accessor,
            shouldBeRemoved);
  ...
  public ContainerController createNewDockerContainer(
      DeploymentContext ctx, DockerComponent comp, boolean shouldBeRemoved)
      throws ContainerException {

    DockerContainerComponents cComponents =
        new DockerContainerComponents(comp, hostContext, client, ctx, dockerConfig);
    accessorInit(cComponents.accessor, cComponents.id);
    ContainerController dc =
        new ErrorAwareContainer<>(
            cComponents.id,
  ...
  public ContainerController createNewRemoteDockerContainer(
      DeploymentContext ctx, RemoteDockerComponent comp, boolean shouldBeRemoved)
      throws ContainerException {

    RemoteDockerContainerComponents cComponents =
        new RemoteDockerContainerComponents(comp, hostContext, client, ctx, dockerConfig);
    accessorInit(cComponents.accessor, cComponents.id);
    ContainerController dc =
        new ErrorAwareContainer<>(
            cComponents.id,
  ...
  private static class LifecycleContainerComponents extends DefaultContainerComponents {
    ...
    LifecycleContainerComponents(
        DeployableComponent comp,
        HostContext hostContext,
        DockerConnector client,
      ...
      this.controller =
          new LifecycleController(comp.getLifecycleStore(), logic, accessor, ec, hostContext);
    }
  }
  ...
  private static class DefaultDockerContainerComponents extends DefaultContainerComponents {
    ...
    DefaultDockerContainerComponents(
  ...
  private static class DockerContainerComponents extends DefaultDockerContainerComponents {
    ...
    DockerContainerComponents(
        DockerComponent comp,
        HostContext hostContext,
        DockerConnector client,
        DeploymentContext ctx,
        DockerConfiguration dockerConfig) {
  ...
  private static class RemoteDockerContainerComponents extends DefaultDockerContainerComponents {
    ...
    RemoteDockerContainerComponents(
        RemoteDockerComponent comp,
        HostContext hostContext,
        DockerConnector client,
        DeploymentContext ctx,
        DockerConfiguration dockerConfig) {
