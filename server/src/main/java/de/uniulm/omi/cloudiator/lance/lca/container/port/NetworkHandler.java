...
public final class NetworkHandler implements DynamicEnvVars {
  ...
  public NetworkHandler(
      ...
      AbstractComponent myComponentParam,
    ...
    for (InPort in : inPortsTmp) {
      ...
      if (state == null)
        throw new IllegalStateException(
            "something went terribly wrong when initialising NetworkHandler");
      for (PortHierarchyLevel level : state) {
  ...
  /**
   * this method loops until information from all required external connection is available (e.g. an
   * application server may require that the database is up and running).
   */
  public void pollForNeededConnections() {
    DownstreamPortUpdater.pollForNeededConnections(outPorts, portAccessor, portHierarchy);
  }

  private void publishLocalAddresses(
      ComponentInstanceId myId, PortRegistryTranslator registryAccessor) throws ContainerException {
    ...
    throw new ContainerException(
        "could register all ports: " + myId + "[" + failed.toString() + "]");
  }

  private void publishLocalPorts(ComponentInstanceId myId, PortRegistryTranslator registryAccessor)
    ...
    for (Map.Entry<String, HierarchyLevelState<Integer>> entry : inPorts.entrySet()) {
      ...
      for (PortHierarchyLevel level : state) {
        try {
          registryAccessor.registerLocalPortAtLevel(
              entry.getKey(), level, state.valueAtLevel(level));
        } catch (RegistrationException de) {
          LOGGER.info("problem when accessing registry", de);
          failed.add(de.getLocalizedMessage());
        }
  ...
  public void startPortUpdaters(LifecycleController controller) {
    DownstreamPortUpdater updater =
        new DownstreamPortUpdater(outPorts, portAccessor, portHierarchy, controller);
  ...
  public void accept(NetworkVisitor visitor) {
    for (Iterator<DynamicEnvVarsImpl> it = currentEnvVarsDynamic.iterator(); it.hasNext(); ) {
      ...
      } else LOGGER.warn("Variables of type: " + impl + "not known.");
    }
