...
public class ErrorAwareContainerLifecycleTest {
  ...
  private static void assertRightState(
      ContainerStatus stat, ErrorAwareContainer<DummyContainer> container) {
    for (ContainerStatus status : ContainerStatus.values()) {
      if (status == stat) assertEquals(status, container.getState());
      else assertFalse(status == container.getState());
    }
  }
  ...
  private void init(boolean creatRegistry) {
    ...
    container =
        new DummyContainer(
            new String[] {null, EnvContextWrapper.getContainerIp()},
  ...
  private ErrorAwareContainer<DummyContainer> linkControllers() {
    ...
    LifecycleController lcc =
        new LifecycleController(creator.build(), interceptor, core.accessor, ctx, null);

    ErrorAwareContainer<DummyContainer> containerWrapper =
        new ErrorAwareContainer<>(
            CoreElements.componentInstanceId, container, core.networkHandler, lcc, core.accessor);
  ...
  public void testContainerCreation() throws RegistrationException, ContainerException {
    ...
    assertTrue(
        container.invocationHistoryMatches(
            DummyContainer.ContainerCalls.LOCAL_ADDRESSES, DummyContainer.ContainerCalls.CREATE));
  }
